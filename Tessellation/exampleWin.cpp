...
#include <iostream>
#include <string>
#include "Windows32bit/Release/Tessellation.h"
using namespace std;
...
namespace Params {
	wstring folder_out = L".";
		// output folder
	wstring name_out = L"test";
		// name of output file, without ext
}

void Test(int begin_number, int iterations, bool remove = true, wstring out_path = L"") {
	Tessellation Tessel;
	...
	if (out_path != L"") {
		Tessel.PrintEdges(Params::folder_out + L"/" + out_path + L".txt");
		wcout << L"saved " << Params::name_out;
	}
	getchar();
}
...
{
	Test(100, 100, true, L"test"); // vysledek 24.165 sec, 1000 pridani, bez odebirani
	//return 0;
	...
	Tessel.PrintEdges(Params::folder_out + L"/" + Params::name_out+L"10con.txt", true);
	// ^ print using connected interpretation ; setting only_once = true has no effect unless some grains have connected form
	Tessel.PrintEdges(Params::folder_out + L"/" + Params::name_out + L"10uncon.txt", false);
